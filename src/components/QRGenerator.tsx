...
const SOCIAL_THEMES = {
  instagram: {
    bgGradient: 'from-[#22c55e] via-[#0d9488] to-[#0284c7]', // Exact Instagram-style as requested in screenshot
    textColor: 'text-[#0d9488] dark:text-[#0d9488]',
  ...
  linkedin: {
    ...
    accentBg: 'bg-[#1d4ed8]/10',
    borderColor: 'border-[#1d4ed8]/20',
    fgColor: '#1d4ed8',
    label: 'LinkedIn'
  },
  github: {
    bgGradient: 'from-[#475569] via-[#334155] to-[#1e293b]',
    textColor: 'text-[#1e293b] dark:text-slate-100',
...
export default function QRGenerator({ onAddHistory }: QRGeneratorProps) {
  ...
  const [socialUsername, setSocialUsername] = useState('himanshubajekan');
  ...
  const [logoPreset, setLogoPreset] = useState<string>('instagram');
  ...
  const [useSocialCard, setUseSocialCard] = useState<boolean>(true);
    ...
    if (format === 'png') {
      ...
      if (isSocialCard) {
        ...
        if (tCtx) {
          ...
          let gradientColors = ['#22c55e', '#0d9488', '#0284c7']; // Default Instagram green/blue
          if (socialPlatform === 'instagram') gradientColors = ['#22c55e', '#0d9488', '#0284c7'];
          else if (socialPlatform === 'linkedin') gradientColors = ['#3b82f6', '#2563eb', '#1d4ed8'];
          else if (socialPlatform === 'github') gradientColors = ['#475569', '#334155', '#1e293b'];
          else if (socialPlatform === 'twitter') gradientColors = ['#0ea5e9', '#0284c7', '#0369a1'];
          ...
          tCtx.fillText(`@${(socialUsername || 'himanshubajekan').toUpperCase()}`, cardX + cardWidth / 2, qrCardY + 410);
    ...
    <div id="qr-generator-section" className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div id="preview-container" className="lg:col-span-5 flex flex-col gap-6 lg:sticky lg:top-24">
        <div className="bg-slate-900/90 dark:bg-slate-950 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden flex flex-col items-center">
          ...
          {/* Layout Mode Selector (Standard vs Social Card) */}
          <div className="flex gap-2 p-1 bg-slate-800/60 rounded-xl mb-6 border border-slate-700/50 w-full max-w-[320px]">
            <button
              type="button"
              onClick={() => setUseSocialCard(false)}
              className={`flex-1 py-1.5 px-3 text-center text-xs font-bold rounded-lg transition-all ${
                !useSocialCard
            ...
            <button
              type="button"
              onClick={() => setUseSocialCard(true)}
              className={`flex-1 py-1.5 px-3 text-center text-xs font-bold rounded-lg transition-all ${
                useSocialCard
                  ? 'bg-gradient-to-r from-teal-500 to-indigo-505 text-white shadow-sm'
                  : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Social Theme
          ...
          {renderError ? (
            ...
            <div className={`relative w-full rounded-2xl bg-gradient-to-b ${SOCIAL_THEMES[socialPlatform as keyof typeof SOCIAL_THEMES]?.bgGradient || 'from-[#22c55e] via-[#0d9488] to-[#0284c7]'} p-6 py-8 flex flex-col items-center justify-center transition-all duration-500 shadow-xl overflow-hidden`}>
              ...
              <div className="bg-white rounded-[28px] p-6 shadow-xl w-full max-w-[260px] flex flex-col items-center gap-4 transition-all duration-300">
                ...
                <div className={`text-center font-display font-black tracking-wider text-sm md:text-base uppercase ${SOCIAL_THEMES[socialPlatform as keyof typeof SOCIAL_THEMES]?.textColor || 'text-[#0d9488]'} truncate max-w-full px-2 mt-1`}>
                  @{socialUsername || 'himanshubajekan'}
                </div>
              </div>
